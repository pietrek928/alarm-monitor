...
struct arm_packet_t {
  ...
  union {
    uint8_t b[8];
    uint64_t w = static_cast<uint64_t>(-1);
  } pass;
  uint32_t partitions = 0;
} PACKED;

[[noreturn]] void network_error(const char *msg) {
  throw std::runtime_error(std::string(msg) + ": " + strerror(errno));
}

void close_socket(int sock) noexcept {
  if (sock != -1) {
    ::close(sock);
  }
}

int alarm_connect(const char *ip, uint16_t port, int timeout_sec = 30) {
...
int recv_data(int sock, uint8_t *buf, int size) {
  ...
  if (r < 0)
    network_error("Receiving data failed");
  return r;
}
...
void send_data(int sock, const uint8_t *buf, int size) {
  ...
  while (sent < size) {
    ...
    if (r <= 0)
      network_error("Sending data failed");
    sent += r;
  }
}
...
uint16_t compute_checksum(const uint8_t *data, int l) {
  ...
  return static_cast<uint16_t>(((r & 0xff) << 8) | ((r >> 8) & 0xff));
}
...
int extract_alarm_packet(const uint8_t *data, int l, int &it,
                         uint8_t *packet_buf, int &packet_state,
...
void AlarmConnection::process_packet(const uint8_t *buf, int size,
                                     std::vector<std::string> &messages_out) {
  if (size < 3)
    return;
  uint16_t chk = compute_checksum(buf, size - 2);
  uint16_t got;
  std::memcpy(&got, buf + size - 2, sizeof(got));
  if (chk != got)
    return;
  size -= 2;
  auto code = static_cast<ALARM_QUERY>(buf[0]);
  buf++;
  size--;
...
AlarmConnection::AlarmConnection() = default;

AlarmConnection::AlarmConnection(const std::string &ip, uint16_t port) {
  connect(ip, port);
}

AlarmConnection &AlarmConnection::operator=(AlarmConnection &&other) noexcept {
  if (this != &other) {
    disconnect();
    sock = other.sock;
    ...
    std::memcpy(packet_buf, other.packet_buf, MAX_PACKET);
...
AlarmConnection::AlarmConnection(AlarmConnection &&other) noexcept {
  *this = std::move(other);
}
...
void AlarmConnection::connect(const std::string &ip, uint16_t port) {
  disconnect();
  sock = alarm_connect(ip.c_str(), port);
}

void AlarmConnection::disconnect() noexcept {
  close_socket(sock);
  sock = -1;
}

std::vector<std::string> AlarmConnection::receive_data() {
  std::vector<std::string> messages_out;
  int r = recv_data(sock, packet_buf, MAX_PACKET);

  int it = 0;
  while (it < r) {
    int packet_len = extract_alarm_packet(
        packet_buf, r, it, packet_buf, packet_state, packet_last_byte);
    if (packet_len > 0) {
...
std::string AlarmConnection::describe_move() {
  return describe_move_state(move_state);
}

void AlarmConnection::query_alarm() {
  send_alarm_query(sock, AL);
}

void AlarmConnection::query_move() {
  send_alarm_query(sock, MOVE);
}

void AlarmConnection::query_armed_partitions() {
  send_alarm_query(sock, ARMED_PARTITIONS_REALLY);
}

void AlarmConnection::send_arm_cmd(uint8_t cmd, uint16_t code,
                                  uint32_t partitions) {
  arm_packet_t p;
  p.cmd = cmd;
  p.pass.b[0] = static_cast<uint8_t>(code >> 8);
  p.pass.b[1] = static_cast<uint8_t>(code);
  p.partitions = partitions;
  uint8_t send_buf[MAX_PACKET];
  int send_len =
      prepare_alarm_packet(send_buf, reinterpret_cast<uint8_t *>(&p), sizeof(p));
  send_data(sock, send_buf, send_len);
}

void AlarmConnection::send_arm(uint16_t code, uint32_t partitions) {
  send_arm_cmd(ARM_MODE_0, code, partitions);
}

void AlarmConnection::send_disarm(uint16_t code, uint32_t partitions) {
  send_arm_cmd(DISARM, code, partitions);
}
